...
class RFServer(RFProtocolFactory, IPC.IPCMessageProcessor):
    ...
    def config_vm_port(self, vm_id, vm_port):
        self.ipc.send(RFCLIENT_RFSERVER_CHANNEL, str(vm_id), 
                      PortConfig(vm_id=str(vm_id), vm_port=str(vm_port), operation_id=str(0)))
    ...
    def register_route_information(self, ri):
        ...
        msg = FlowMod(dp_id=str(entry.dp_id),
            address=str(ri.address),
            netmask=str(ri.netmask),
            dst_port=str(ri.dst_port),
            src_hwaddress=str(ri.src_hwaddress),
            dst_hwaddress=str(ri.dst_hwaddress),
    ...
    def send_datapath_config_message(self, dp_id, operation_id):
        self.ipc.send(RFSERVER_RFPROXY_CHANNEL, RFPROXY_ID, 
                      DatapathConfig(dp_id=str(dp_id), operation_id=str(operation_id)))
