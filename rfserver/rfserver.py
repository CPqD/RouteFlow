...
class RFServer(IPC.IPCMessageFactory, IPC.IPCMessageProcessor):
    ...
    def process(self, from_, to, channel, msg):
        type_ = msg.get_type()
        print "Got message type=%s" % str(type_)
        if type_ == PORT_REGISTER:
            vm_id = int(msg["vm_id"])
            vm_port = int(msg["vm_port"])

            self.register_vm_port(vm_id, vm_port)
            ...
            flm = FlowMod(dp_id=str(entry.dp_id),
                          address=msg["address"],
                          netmask=msg["netmask"],
                          dst_port=msg["dst_port"],
                          src_hwaddress=msg["src_hwaddress"],
                          dst_hwaddress=msg["dst_hwaddress"],
                          is_removal=msg["is_removal"])
            self.ipc.send(RFSERVER_RFPROXY_CHANNEL, RFPROXY_ID, flm);
        elif type_ == DATAPATH_PORT_REGISTER:
            dp_id = int(msg["dp_id"])
            dp_port = int(msg["dp_port"])
            
            self.register_dp_port(dp_id, dp_port);
        elif type_ == DATAPATH_DOWN:
            dp_id = int(msg["dp_id"])
            self.set_dp_down(dp_id)
        elif type_ == PORT_MAP:
            vm_id = int(msg["vm_id"])
            vm_port = int(msg["vm_port"])
            vs_id = int(msg["vs_id"])
            vs_port = int(msg["vs_port"])
            
            self.map_port(vm_id, vm_port, vs_id, vs_port)
        else:
            return False
        return True
    ...
    def config_vm_port(self, vm_id, vm_port):
        self.ipc.send(RFCLIENT_RFSERVER_CHANNEL, vm_id, 
                      PortConfig(vm_id=vm_id, vm_port=vm_port))

    def send_datapath_config_message(self, dp_id, operation):
        self.ipc.send(RFSERVER_RFPROXY_CHANNEL, RFPROXY_ID, 
                      DatapathConfig(dp_id=dp_id, operation=operation))
        
    def config_dp(self, dp_id):
        # TODO: consider the case in which the dp tables need to be cleared and restored
...
s.map_port(2192192192192192, 2, 42, 43)
print str(s)
